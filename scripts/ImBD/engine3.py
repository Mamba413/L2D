...
def evaluate_model_SPO(model, data, DEVICE):
    ...
    with torch.no_grad():
        for batch in tqdm.tqdm(eval_loader, desc="Evaluating"):
            ...
            output = model(text)
            eval_time_list.append((time.perf_counter() - start) / 2)
...
def run(
    ...
    if args.ebt or args.eval_only:
        ...
        if args.SPOtrained:
            output_path = f"{args.output_file}.adarewritegpt_{args.criterion_fn}.json"
        else:
    ...
    if args.eval_after_train:
        ...
        if args.SPOtrained:
            output_path = f"{args.output_file}.adarewritegpt_{args.criterion_fn}.json"
            # output_path = f"{args.output_file}.adarewritegpt_{args.criterion_fn}.rewrite_{args.regen_number}.json"
        else:
            method_name=args.base_model.split("_")[-1]
            output_path = f"{args.output_file}.{method_name}.json"
        with open(output_path, "w") as j:
